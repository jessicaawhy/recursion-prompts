...
var palindrome = function(string) {
  ...
  if (string[0].toLowerCase() !== string[length - 1].toLowerCase()) {
    return false;
  }
...
var fizzBuzz = function(n) {
  ...
  if (n > 1) {
    var prior = fizzBuzz(n - 1);
    return [].concat(prior, result);
  }
...
var rMap = function(array, callback) {
  var curr = array.slice(0,1);
  curr[0] = callback(curr[0]);
  var next = array.slice(1);

  if (next.length === 0) {
    return curr;
  } else {
    return curr.concat(rMap(next, callback));
  }
};
...
var countKeysInObj = function(obj, key) {
  ...
  for (var keyname in obj) {
    ...
    if (typeof obj[keyname] === 'object') {
      count += countKeysInObj(obj[keyname], key);
    }
  }
